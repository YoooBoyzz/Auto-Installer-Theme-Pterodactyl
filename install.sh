...
check_token() {
  ...
  if [ "$USER_TOKEN" = "YoooXyz" ]; then
    ...
    echo -e "${YELLOW}TELEGRAM : @YoooAvailable${NC}"
    echo -e "${YELLOW}WHATSAPP : +6283128730972${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© 2024 Yooo.${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/YoooBoyzz/Auto-Installer-Theme-Pterodactyl/raw/main/stellar.zip")        
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/YoooBoyzz/Auto-Installer-Theme-Pterodactyl/raw/main/billing.zip")
        break
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/YoooBoyzz/Auto-Installer-Theme-Pterodactyl/raw/main/enigma.zip")
        break
        ;; 
      x)
        return
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Yooo         ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : YoooAvailable      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Yooo            ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Yooo  ${NC}"
